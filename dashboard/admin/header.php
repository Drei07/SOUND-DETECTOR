...
$stmt = $user->runQuery("SELECT * FROM sensors WHERE id=:id");
$stmt->execute(array(":id" => 1)); // Fixed parameter placeholder

$sensorData = $stmt->fetch(PDO::FETCH_ASSOC); // Fetch only one row

if ($sensorData) {
    $maxSound        = $sensorData['maxSound'];
    $maxCount        = $sensorData['maxCount'];
